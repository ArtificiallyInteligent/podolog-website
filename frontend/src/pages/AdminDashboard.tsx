...
const AdminDashboard: React.FC = () => {
	...
	const handleServiceSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			if (!response.ok) {
				...
				throw new Error(
					errorData.error || "Nie udało się zapisać usługi"
				);
			}
	...
	const handleServiceDelete = async (serviceId: number) => {
		...
		try {
			...
			if (!response.ok) {
				const errorData = await response.json().catch(() => ({}));
				throw new Error(
					errorData.error || "Nie udało się usunąć usługi"
				);
			}
			await Promise.all([fetchServices(), fetchSummary()]);
		} catch (error) {
	...
	const handleCategoryEdit = (category: ServiceCategory) => {
		...
		// Scrolluj do formularza kategorii
		setTimeout(() => {
			const categoryFormElement =
				document.getElementById("category-form-card");
			if (categoryFormElement) {
				categoryFormElement.scrollIntoView({
					behavior: "smooth",
					block: "center",
				});
			}
	...
	const handleCategorySubmit = async (e: React.FormEvent) => {
		...
		try {
			const url = editedCategoryId
				? `/api/service-categories/${editedCategoryId}`
				: "/api/service-categories";
			const method = editedCategoryId ? "PUT" : "POST";
			const response = await fetch(url, {
				method,
				headers: { "Content-Type": "application/json" },
			...
			if (!response.ok) {
				const errorData = await response.json().catch(() => ({}));
				throw new Error(
					errorData.error || "Nie udało się utworzyć kategorii"
				);
			}
	...
	const updateAppointmentStatus = async (
		...
		try {
			...
			if (!response.ok) {
				...
				throw new Error(
					errorData.error ||
						"Nie udało się zaktualizować statusu rezerwacji"
				);
			}
	...
	const deleteAppointment = async (appointmentId: number) => {
		...
		try {
			...
			if (!response.ok) {
				const errorData = await response.json().catch(() => ({}));
				throw new Error(
					errorData.error || "Nie udało się usunąć rezerwacji"
				);
			}
					...
					<div className="border-t border-slate-800 px-6 py-6 text-xs text-slate-500">
						...
						<ul className="mt-3 space-y-2">
							<li className="flex gap-2">
								...
								Potwierdzaj rezerwacje, aby automatycznie
								wysyłać komunikaty do pacjentów.
							</li>
							<li className="flex gap-2">
								<span className="mt-1 block h-2 w-2 rounded-full bg-sky-400"></span>{" "}
								Aktualizuj ceny w sekcji usług, by cennik na
								stronie był zawsze aktualny.
							</li>
						</ul>
					</div>
				</aside>
					...
					>
						<div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
							<div>
								...
								<p className="mt-3 max-w-xl text-sm text-slate-400">
									Monitoruj rezerwacje, aktualizuj ofertę
									usług, zarządzaj cennikiem i utrzymuj
									najwyższy poziom obsługi pacjenta w jednym
									miejscu.
								</p>
							</div>
								...
								<CardHeader className="flex flex-row items-center justify-between gap-4">
									<div>
										...
										<p className="mt-1 text-sm text-slate-400">
											Monitoruj status wizyt i planuj
											harmonogram gabinetu
										</p>
									</div>
							...
							<Card className="border border-slate-800/70 bg-slate-900/70">
								<CardHeader className="flex flex-row items-center justify-between gap-4">
									<div>
										...
										<p className="text-sm text-slate-400">
											Aktualizuj listę usług, by oferta na
											stronie była zawsze aktualna.
										</p>
									</div>
									<div className="hidden rounded-full border border-slate-800 bg-slate-950/60 px-4 py-2 text-xs text-slate-400 md:flex md:items-center md:gap-2">
										...
										{services.length} pozycji w ofercie
									</div>
								</CardHeader>
								<CardContent className="space-y-4">
							...
							<div className="space-y-6">
								<Card className="border border-slate-800/70 bg-slate-900/70">
									<CardHeader>
										...
										<p className="text-sm text-slate-400">
											Uzupełnij informacje, aby oferta
											natychmiast pojawiła się na stronie.
										</p>
									</CardHeader>
									<CardContent>
										<form
								...
								<Card
									className="border border-slate-800/70 bg-slate-900/70"
								...
								>
									<CardHeader>
										...
										<p className="text-sm text-slate-400">
											{editedCategoryId
												? "Zaktualizuj dane kategorii usług"
												: "Grupuj zabiegi, aby pacjentom łatwiej było znaleźć interesujące ich usługi."}
										</p>
									</CardHeader>
									<CardContent>
										<form
						...
						<section className="space-y-6" id="pricing">
							<div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
								<div>
									...
									<p className="mt-1 text-sm text-slate-400">
										Zmiany w panelu są od razu widoczne na
										stronie – sprawdź jak prezentuje się
										oferta.
									</p>
								</div>
								<div className="flex items-center gap-2 text-xs text-slate-500">
									<List className="h-4 w-4" />{" "}
									{services.length} pozycji • Średnia cena:{" "}
									{currencyFormatter.format(
											...
											<div className="flex items-start justify-between">
												...
												<div className="flex items-center gap-2">
													<Button
														size="sm"
														variant="ghost"
														className="text-slate-400 hover:bg-blue-500/10 hover:text-blue-300"
														onClick={() =>
															handleCategoryEdit(
																category
															)
						...
						<footer className="border-t border-slate-900/60 pt-6 text-xs text-slate-500">
							<p>
								...
								Podologiczny. Panel administracyjny – wersja
								beta.
							</p>
						</footer>
					</div>
