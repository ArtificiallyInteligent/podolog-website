  ...
  return (
    ...
    >
      <div className="relative w-full overflow-hidden rounded-3xl bg-transparent transition-colors duration-300">
        <div className="p-8 min-h-[600px] w-full relative">
...
function StepsNav({ steps: stepItems, current, onChange }: { steps: readonly Step[]; current: number; onChange: (index: number) => void; }) {
  return (
    <nav aria-label="Progress" className="flex justify-center px-4 mb-8">
      <ol className="flex w-full flex-wrap items-center justify-center gap-6" role="list">
...
export function FeatureCarousel({ categories, allServices }: FeatureCarouselProps) {
  const { currentNumber: currentCategoryIndex, setStep } = useNumberCycler(categories.length)
  ...
  return (
    <div className="flex flex-col gap-8 w-full max-w-6xl mx-auto p-4">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
        <StepsNav current={currentCategoryIndex} onChange={setStep} steps={categories.map(cat => ({ id: cat.id, name: cat.name, title: cat.name, description: '' }))} />
      </motion.div>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {currentServices.slice(0, 4).map((service, index) => {
              ...
              const animationDirection = isLeftSide ? { x: -50 } : { x: 50 };
